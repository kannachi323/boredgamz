...
export default function GomokuGame() {
  ...
  const showReconnectBanner =
    ...
    (connectionStatus === "error" || (!conn && connectionStatus !== "idle"));
  ...
  return (
    <div className="h-[90vh] w-full grid grid-cols-26 gap-6 p-6 bg-[#1b1918] overflow-hidden">
      ...
      {showReconnectBanner && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-[#433d3a] border border-[#C3B299] rounded-lg px-4 py-2 flex items-center gap-3 text-[#C3B299]">
          <p className="text-sm font-semibold">
            {connectionError || "Connection interrupted. Retry to continue this match."}
          </p>
          <button
            className="px-3 py-1 text-sm font-bold bg-[#C3B299] text-[#433d3a] rounded hover:bg-[#d7c9b8] transition"
