...
func InsertGame(db *db.Database, gameID string, player1ID string, player2ID string, gameState *model.ConnectFourGameStateRow) error {
	return SaveGameWithMessages(db, gameID, player1ID, player2ID, gameState, gameState.Messages)
}

func SaveGameWithMessages(db *db.Database, gameID string, player1ID string, player2ID string, gameState *model.ConnectFourGameStateRow, messages []*model.ChatMessage) error {
	...
	_, err = tx.Exec(ctx, query, gameID, gameType, player1ID, player2ID, gameStateBytes)
	if err != nil {
		return fmt.Errorf("failed to insert connect four game: %w", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("failed to commit connect four transaction: %w", err)
	}

	return nil
}

func GetGameByID(db *db.Database, gameID string) (*model.ConnectFourGameStateRow, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	var gameState model.ConnectFourGameStateRow
...
func GetGamesByPlayerID(db *db.Database, playerID string) ([]*model.ConnectFourGameStateRow, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	var games []*model.ConnectFourGameStateRow
	for rows.Next() {
		var gameID string
		var rawJSON []byte
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("connect four rows error: %w", err)
	}
...
func insertMessages(ctx context.Context, exec dbExecutor, gameID string, messages []*model.ChatMessage) error {
	...
	for _, message := range messages {
		...
		if _, err := exec.Exec(ctx, query, gameID, gameType, message.SenderID, message.SenderName, message.Content, sentAt); err != nil {
			return fmt.Errorf("failed to insert connect four message: %w", err)
		}
	}
...
func getMessagesByGameID(ctx context.Context, queryer dbQuerier, gameID string) ([]*model.ChatMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to query connect four messages: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&senderID, &senderName, &content, &sentAt); err != nil {
			return nil, fmt.Errorf("failed to scan connect four message: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("connect four message rows error: %w", err)
	}
